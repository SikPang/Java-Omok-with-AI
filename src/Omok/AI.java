/*	** 해야 할 것 **
 * 	1. 17*17 이상의 부분일 때, 판정 오류
 *  2. 플레이어가 돌을 두면 바로바로 흰돌이 둬지는 것이 아닌 시간차를 두게하는 알고리즘 구현 (흑,백이 모두 끝나야 screenDraw가 돌아감)
 *  3. 삼삼 금지 알고리즘 추가 여부
 *  
 *  *. reCheck로 인해 211102로 막았는데도 0에 또 막음 (행에서 발견, 추가 문제 시 참고)
 *  *. 11011, 11101, 10111 알고리즘이 returnNum이 1111이면 패스되게 되어있음 (문제 시 참고)
 */
		...
		*/
		//if(map.endTurn == true) {
			...
			if(reCheck==true)
				currentAI = CheckAI();
			for(int i=0; i<19; i++) {
				if (stopAlphaOh) break;		// 두 번쨰 for문 빠져나가기 (한 번 작동할 때 여러번의 상태 변경을 방지하기 위해 for 문을 두 번 빠져나가기 위한 코드. return 기능)
				for(int j=0; j<19; j++) {
